...
def solve_heat_equation(
    ...
    A=_build_matrix(cfg, dx, dy, dt) #bygger matrise A


    for k in range(cfg.nt-1): #iterere over hele "rommet" i tiden
        b=_build_rhs(cfg, T[k], X, Y, dx, dy, dt, t[k+1]) #bygger høyresiden b
        T_next = np.linalg.solve(A,b) #løser A*T^(k+1)=b, der T^(k+1) er lagret som 1D vektor med lengde nx*ny
        T[k+1,:,:] = T_next.reshape(cfg.nx,cfg.ny) #omformer 1D vektor til 2D matrise og lagerer i T
